...
slavery_index <- slavery_index |>
  rename(Per_100000 = Estimated.prevalence.of.modern.slavery.per.1.000.population) |>
...
slavery_index$Per_100000 <- slavery_index$Per_100000 * 100
```
...
```{r}
country_max <- slavery_index[which.max(slavery_index$Per_100000), ]
country_min <- slavery_index[which.min(slavery_index$Per_100000), ]

slavery_index |> arrange(desc(Per_100000))

slavery_index |> filter(Region == "Asia and the Pacific") |> arrange(desc(Per_100000))
...
top_10 <- slavery_index |> arrange(desc(Per_100000)) |> head(n=10)
```


```{r}
grouped <-human_trafficking %>%
  group_by(Country) %>%
...
Myanmar <- ts(grouped %>%
            ...
            select(c("Year", "Detected_Number")) %>%
            subset(select=c(Year, Detected_Number)))
